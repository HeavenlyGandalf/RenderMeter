...
interface Point { key: string; type: SummaryType; vars: Record<string, string | number>; }
...
function analyze(run: BenchmarkRun): Point[] {
    ...
    for (const [engine, m] of entries) {
      ...
      if (spread > 5) {
        points.push({
          ...
          vars: { engine, scenario, min: m.min.toFixed(3), max: m.max.toFixed(3), spread: spread.toFixed(1) },
        });
      }
    }
  }
  ...
  if (sorted.length > 0 && run.results.length > 1) {
    ...
    points.push({ key: 'summary.overall', type: 'info', vars: { winner, count, total: run.results.length } });
  }
